...
import { TrendingUp, TrendingDown, Users, ShoppingCart, IndianRupee, Package, AlertTriangle } from 'lucide-react';
  ...
  const analytics = useMemo(() => {
    ...
    deliveredOrders.forEach((o) => {
      o.items.forEach((item) => {
        ...
        const existing = productRevMap.get(pid) || { name: item.product.name, revenue: 0, units: 0 };
        existing.revenue += item.product.mrp * item.quantity;
        existing.units += item.quantity;
        productRevMap.set(pid, existing);
      });
    });
    const topProducts = Array.from(productRevMap.values()).sort((a, b) => b.revenue - a.revenue).slice(0, 5);

    const lowStockProducts = products.filter((p) => p.stock > 0 && p.stock < 50);
    const outOfStock = products.filter((p) => p.stock === 0);

    const monthlyRevenue: { month: string; B2C: number; B2B: number }[] = [];
    for (let i = 5; i >= 0; i--) {
      ...
      monthlyRevenue.push({ month: monthLabel, B2C: b2c, B2B: b2b });
    }

    const dailyOrders: { day: string; orders: number }[] = [];
    for (let i = 6; i >= 0; i--) {
      const d = new Date(); d.setDate(d.getDate() - i);
      dailyOrders.push({ day: d.toLocaleDateString('en-IN', { weekday: 'short' }), orders: orders.filter((o) => o.date === d.toISOString().split('T')[0]).length });
    }

    const categoryRevMap = new Map<string, number>();
    orders.forEach((o) => { o.items.forEach((item) => { categoryRevMap.set(item.product.category, (categoryRevMap.get(item.product.category) || 0) + item.product.mrp * item.quantity); }); });
    ...
    const categoryRevenue = Array.from(categoryRevMap.entries())
      .map(([slug, revenue]) => ({ name: CATEGORIES.find((c) => c.slug === slug)?.name || slug, value: Math.round((revenue / totalCatRev) * 100), revenue }))
      .sort((a, b) => b.revenue - a.revenue).slice(0, 6);

    return { totalOrders, totalRevenue, aov, revChange, returningCustomers, statusCounts, topProducts, lowStockProducts, outOfStock, monthlyRevenue, dailyOrders, categoryRevenue };
  }, [orders, registeredUsers]);

  const stats = [
    { label: 'Total Revenue', value: analytics.totalRevenue >= 100000 ? `Rs.${(analytics.totalRevenue / 100000).toFixed(1)}L` : `Rs.${analytics.totalRevenue.toLocaleString()}`, change: analytics.revChange !== 0 ? `${analytics.revChange > 0 ? '+' : ''}${analytics.revChange}%` : '—', up: analytics.revChange >= 0, icon: IndianRupee },
    { label: 'Avg Order Value', value: `Rs.${analytics.aov.toLocaleString()}`, change: `${analytics.totalOrders} orders`, up: true, icon: ShoppingCart },
    { label: 'Returning Customers', value: `${analytics.returningCustomers}`, change: `of ${new Set(orders.map((o) => o.userName)).size} total`, up: true, icon: Users },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border rounded-xl p-4 sm:p-5">
        <h3 className="text-[15px] sm:text-[16px] mb-4" style={{ fontWeight: 600 }}>Revenue: B2C vs B2B</h3>
        ...
        <div className="flex gap-6 mt-3 justify-center text-[12px] sm:text-[13px]">
          <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-[#16a34a]" /> B2B</div>
          <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-[#0ea5e9]" /> B2C</div>
        </div>
      </div>
            ...
            {analytics.topProducts.map((p, i) => (
              <div key={p.name} className="flex items-center gap-3 sm:gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 text-[11px] text-muted-foreground mt-0.5"><span>{p.units} units</span><span>Rs.{p.revenue.toLocaleString()}</span></div>
                </div>
