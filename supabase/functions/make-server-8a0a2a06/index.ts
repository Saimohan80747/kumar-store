...
// ─── Password hashing via Web Crypto (SHA-256 + hex) ───

async function hashPassword(password: string): Promise<string> {
  const encoder = new TextEncoder();
  const data = encoder.encode(password + "kumarstore_salt_2026");
...
function orderToFrontend(row: any) {
  if (!row) return null;
  // Extract metadata from items array (backward-compatible storage)
  const items = row.items || [];
  const metaEntry = items.find((i: any) => i._meta === true);
  const orderItems = items.filter((i: any) => !i._meta);
  return {
    id: row.id,
    items: orderItems,
    total: Number(row.total),
    deliveryFee: row.delivery_fee != null ? Number(row.delivery_fee) : (metaEntry?.deliveryFee ?? 0),
    status: row.status,
    date: row.order_date,
    ...
    couponCode: row.coupon_code || metaEntry?.couponCode || undefined,
    couponDiscount: row.coupon_discount != null ? Number(row.coupon_discount) : (metaEntry?.couponDiscount ?? undefined),
  };
}

function orderToDB(order: any) {
  // Store metadata inside items JSONB for backward compatibility
  const meta = {
    _meta: true,
  ...
  const itemsWithMeta = [...(order.items || []), meta];
  const dbOrder: any = {
    id: order.id,
...
app.post(`${PREFIX}/migrate`, async (c) => {
  try {
    const db = supabase();
    // Using rpc to run raw SQL via Supabase's pg_net or sql function
    // Since we can't run raw SQL via PostgREST, we'll use a safe approach:
    // Try inserting a test order with new columns — if they don't exist, we'll
    // fall back to storing coupon/delivery data inside the JSONB items column.
    // Better approach: use Supabase management API or just handle gracefully.

    // Test if new columns exist by selecting them
    const { data, error } = await db
      .from("kumar_orders")
      .select("delivery_fee, coupon_code, coupon_discount")
      .limit(1);

    const migrationNeeded = error && error.message.includes("does not exist");

    // Also update admin password to hashed version if it's still plain text
    const { data: admin } = await db
      .from("kumar_shop_users")
      .select("id, password")
      .eq("role", "admin")
      .eq("email", "admin@kumarstore.com")
      .maybeSingle();
    ...
    if (migrationNeeded) {
      return c.json({
        ...
        message: "New columns (delivery_fee, coupon_code, coupon_discount) need to be added to kumar_orders table. Please run the following SQL in Supabase SQL Editor:",
        sql: `ALTER TABLE kumar_orders ADD COLUMN IF NOT EXISTS delivery_fee NUMERIC DEFAULT 0;
ALTER TABLE kumar_orders ADD COLUMN IF NOT EXISTS coupon_code TEXT DEFAULT NULL;
ALTER TABLE kumar_orders ADD COLUMN IF NOT EXISTS coupon_discount NUMERIC DEFAULT NULL;
ALTER TABLE kumar_orders ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT now();`,
      });
    }

    return c.json({ status: "ok", adminPasswordUpdated, message: "All columns exist" });
  } catch (err) {
    return c.json({ error: `Migration check failed: ${err}` }, 500);
  }
});
    ...
    const { error: custErr } = await db.from("kumar_customer_users").upsert([
      {
        ...
        password: custPass, address: "123 Main Street, Mumbai 400001", total_profit: 0,
      },
    ]);
    ...
    const { error: shopUsersErr } = await db.from("kumar_shop_users").upsert([
      {
        ...
        role: "shopowner", password: shopPass, shop_name: "Raj Kirana Store",
      ...
      },
      {
        ...
        role: "admin", password: adminPass, total_profit: 0,
      },
    ]);
    if (shopUsersErr) console.log("Seed shop_users error:", shopUsersErr);

    // Seed shop requests
    const shopReqPass = await hashPassword("password123");
    const { error: reqsErr } = await db.from("kumar_shop_requests").upsert([
      { id: "sr-1", name: "Sanjay Patel", email: "sanjay@store.com", phone: "9876543001", shop_name: "Sanjay General Store", shop_location: "Ahmedabad, Gujarat", password: shopReqPass, status: "pending", request_date: "2026-03-01" },
      { id: "sr-2", name: "Krishna Kumar", email: "krishna@mart.com", phone: "9876543002", shop_name: "New Krishna Mart", shop_location: "Pune, Maharashtra", password: shopReqPass, status: "pending", request_date: "2026-03-02" },
      { id: "sr-3", name: "Ramesh Sharma", email: "ramesh@traders.com", phone: "9876543003", shop_name: "Balaji Traders", shop_location: "Delhi, NCR", password: shopReqPass, status: "pending", request_date: "2026-03-03" },
    ]);
    if (reqsErr) console.log("Seed shop requests error:", reqsErr);

    // Seed orders with realistic item data
    const sampleCustomerItems = [
      { product: { id: 'p1', name: 'Surf Excel Matic Top Load', brand: 'Surf Excel', category: 'soaps-detergents', image: '', mrp: 399, purchasePrice: 245, shopPrice: 310, customerPrice: 363, minWholesaleQty: 12, stock: 500, sku: 'SE-TL-001', unitType: 'Piece', featured: true, rating: 4.5, reviews: 234, description: '' }, quantity: 2 },
      { product: { id: 'p2', name: 'Tata Gold Tea 500g', brand: 'Tata Tea', category: 'tea-coffee', image: '', mrp: 285, purchasePrice: 175, shopPrice: 220, customerPrice: 259, minWholesaleQty: 24, stock: 800, sku: 'TT-G-002', unitType: 'Piece', featured: true, rating: 4.7, reviews: 567, description: '' }, quantity: 1 },
    ];
    const sampleShopItems = [
      { product: { id: 'p5', name: 'Aashirvaad Atta 10kg', brand: 'Aashirvaad', category: 'grocery-essentials', image: '', mrp: 499, purchasePrice: 330, shopPrice: 410, customerPrice: 463, minWholesaleQty: 10, stock: 300, sku: 'AA-AT-005', unitType: 'Piece', featured: true, rating: 4.8, reviews: 2134, description: '' }, quantity: 10 },
      { product: { id: 'p12', name: 'Fortune Soyabean Oil 5L', brand: 'Fortune', category: 'grocery-essentials', image: '', mrp: 799, purchasePrice: 540, shopPrice: 680, customerPrice: 751, minWholesaleQty: 6, stock: 200, sku: 'FT-SO-012', unitType: 'Piece', featured: true, rating: 4.6, reviews: 1567, description: '' }, quantity: 6 },
    ];
    const { error: ordersErr } = await db.from("kumar_orders").upsert([
      { id: "ORD-10041", user_id: "cust-1", user_name: "Priya Sharma", user_role: "customer", items: [...sampleCustomerItems, { _meta: true, deliveryFee: 0, couponCode: null, couponDiscount: null }], total: 1034, status: "delivered", payment_method: "upi", order_date: "2026-02-25" },
      { id: "ORD-10042", user_id: "shop-1", user_name: "Raj Kumar", user_role: "shopowner", items: [...sampleShopItems, { _meta: true, deliveryFee: 0, couponCode: null, couponDiscount: null }], total: 8180, status: "shipped", payment_method: "credit", order_date: "2026-02-28" },
      { id: "ORD-10043", user_id: "cust-1", user_name: "Priya Sharma", user_role: "customer", items: [sampleCustomerItems[0], { _meta: true, deliveryFee: 49, couponCode: null, couponDiscount: null }], total: 775, status: "placed", payment_method: "cod", order_date: "2026-03-03" },
    ]);
...
app.post(`${PREFIX}/users`, async (c) => {
  try {
    ...
    return c.json({ success: true, user: { id: user.id, name: user.name, email: user.email, phone: user.phone, role: 'customer', address: user.address, profit: 0 } });
  } catch (err) {
...
app.post(`${PREFIX}/auth/login`, async (c) => {
  try {
    ...
    if (role === "shopowner") {
      ...
      if (shopUser && await verifyPassword(password, shopUser.password)) {
...
app.post(`${PREFIX}/shop-requests`, async (c) => {
  try {
    ...
    // Hash the password before storing the request
    const hashedPassword = await hashPassword(request.password);
    const requestWithHash = { ...request, password: hashedPassword };
    const { error } = await db.from("kumar_shop_requests").insert(shopReqToDB(requestWithHash));
    if (error) {
...
app.put(`${PREFIX}/shop-requests/:id/approve`, async (c) => {
  try {
    ...
    // Create new shop user from the request (password is already hashed in the request)
    const newUser = {
      id: `shop-${Date.now()}`,
      name: request.name,
      email: request.email,
...
app.get(`${PREFIX}/orders`, async (c) => {
  try {
    const db = supabase();
    // Support optional user_id filter  e.g. /orders?user_id=cust-1
  ...
  try {
    ...
    if (updates.status) {
      ...
      if (currentOrder) {
        ...
        if (currentOrder.status !== updates.status && !allowed.includes(updates.status)) {
          return c.json(
            { error: `Cannot transition from '${currentOrder.status}' to '${updates.status}'` },
            400
          );
        }
      }
    ...
    // Handle items + metadata updates via JSONB _meta approach
    const hasMetaUpdate = updates.deliveryFee !== undefined || updates.couponCode !== undefined || updates.couponDiscount !== undefined;
    if (updates.items || hasMetaUpdate) {
      // Fetch current items to preserve or update _meta
      const { data: currentOrder } = await db
        .from("kumar_orders")
        .select("items")
        .eq("id", id)
      ...
      const currentMeta = currentItems.find((i: any) => i._meta === true) || { _meta: true };
      const baseItems = (updates.items || currentItems).filter((i: any) => !i._meta);
      const newMeta = {
        ...currentMeta,
        ...(updates.deliveryFee !== undefined ? { deliveryFee: updates.deliveryFee } : {}),
        ...(updates.couponCode !== undefined ? { couponCode: updates.couponCode } : {}),
        ...(updates.couponDiscount !== undefined ? { couponDiscount: updates.couponDiscount } : {}),
      };
      dbUpdates.items = [...baseItems, newMeta];
    }
...
app.get(`${PREFIX}/analytics`, async (c) => {
  try {
    ...
    const pending = orders.filter((o: any) => ["placed", "accepted", "shipped"].includes(o.status));
    const totalRevenue = delivered.reduce((s: number, o: any) => s + (o.total || 0), 0);
    const pipelineValue = pending.reduce((s: number, o: any) => s + (o.total || 0), 0);
    ...
    // Compute profit from delivered orders (revenue - purchase cost of items)
    const totalProfit = delivered.reduce((sum: number, o: any) => {
      const cost = (o.items || []).reduce((s: number, i: any) => s + (i.product?.purchasePrice || 0) * (i.quantity || 0), 0);
    ...
    // B2B vs B2C split
    const b2bOrders = orders.filter((o: any) => o.userRole === "shopowner").length;
    const b2cOrders = orders.filter((o: any) => o.userRole !== "shopowner").length;
    const b2bRevenue = delivered.filter((o: any) => o.userRole === "shopowner").reduce((s: number, o: any) => s + (o.total || 0), 0);
    const b2cRevenue = delivered.filter((o: any) => o.userRole !== "shopowner").reduce((s: number, o: any) => s + (o.total || 0), 0);
